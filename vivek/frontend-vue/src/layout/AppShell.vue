...
<template>
  <div class="min-h-screen text-foreground">
    <header class="h-16 border-b/70 glass-panel px-4 lg:px-6 flex items-center justify-between sticky top-0 z-50">
      ...
      <div class="flex items-center gap-2">
        <Button variant="outline" type="button" class="h-9 px-3" @click="cycleTheme">
          Theme: {{ themeLabel }}
        </Button>
        <Button
        ...
        <Button
          ...
          variant="outline"
          type="button"
          class="h-9 px-3 inline-flex items-center gap-1.5"
          @click="logout"
        >
    ...
    <div class="h-[calc(100svh-4rem)] flex">
      <aside
      ...
      <aside v-if="showMobileNav" class="md:hidden fixed top-14 left-0 w-64 h-[calc(100svh-3.5rem)] glass-panel border-r p-4 z-50">
        <div class="space-y-2">
      ...
      <main class="flex-1 flex min-h-0 overflow-hidden">
        <section
          class="flex-1 min-h-0 p-4 lg:p-6 bg-background/30 backdrop-blur"
          :class="isChatRoute ? 'overflow-hidden min-h-0' : 'overflow-y-auto min-h-0'"
        >
          <RouterView />
        </section>
